...
public:
  ...
  float getNormal() { return dis_normal(gen); }
...
struct Ray
{
  Ray(const Point origin, const Vec direction)
    : orig(origin), dir(direction / cv::norm(direction)), color(0, 0, 0), throughput(1.0, 1.0, 1.0)
  {
  }
...
struct Sphere : Object
{
  ...
  Sphere(const Point& pos, float radius, const Material& material)
    : pos(pos), radius(radius), mat(material)
  {
  }
  ...
  const Material& getMaterial() const override { return mat; }
};
...
Scene buildSceneCornellBox(bool quad_lighting = true)
{
  ...
  if (quad_lighting) {
    ...
    scene.push_back(std::make_shared<Plane>(
        Point(0, -1.25, 0), Vec(0, 1, 0), makePassiveMaterial(GRAY0))); // Ceiling
  } else {
    // Light + ceiling (cheaper alternative)
    scene.push_back(std::make_shared<Plane>(
        Point(0, -1.25, 0),
        Vec(0, 1, 0),
        Material(LIGHT_YELLOW0, 2.0, BLACK, 0.0, 1.0, LIGHT_YELLOW0)));
  }

  // Walls
  scene.push_back(
      std::make_shared<Plane>(Point(0, 0, 2.5), Vec(0, 0, -1), makePassiveMaterial(GRAY0))); // Back
  scene.push_back(
      std::make_shared<Plane>(Point(-1.25, 0, 0), Vec(1, 0, 0), makePassiveMaterial(RED0))); // Left
  scene.push_back(std::make_shared<Plane>(
      Point(1.25, 0, 0), Vec(-1, 0, 0), makePassiveMaterial(GREEN0))); // Right
  scene.push_back(std::make_shared<Plane>(
      Point(0, 1.25, 0), Vec(0, -1, 0), makePassiveMaterial(GRAY0))); // Floor
  scene.push_back(
      std::make_shared<Plane>(Point(0, 0, -1.6), Vec(0, 0, 1), makePassiveMaterial(BLACK))); // Rear
...
Scene buildSceneCornellBoxQuads()
{
  ...
  scene.push_back(std::make_shared<Quad>(
      ...
      right_top_far,
      right_top_near,
      makePassiveMaterial(GRAY0))); // Ceiling
  scene.push_back(std::make_shared<Quad>(
      left_top_far,
      right_top_far,
      right_bottom_far,
      left_bottom_far,
      makePassiveMaterial(GRAY0))); // Back
  scene.push_back(std::make_shared<Quad>(
      left_top_near,
      left_top_far,
      left_bottom_far,
      left_bottom_near,
      makePassiveMaterial(RED0))); // Left
  // scene.push_back(std::make_shared<Triangle>(
  //     left_top_near, left_top_far, left_bottom_far, makePassiveMaterial(RED0))); // Left (up)
  // scene.push_back(std::make_shared<Triangle>(
  //     left_bottom_far, left_bottom_near, left_top_near, makePassiveMaterial(RED0))); // Left
  //     (down)
  scene.push_back(std::make_shared<Quad>(
      right_top_near,
      right_top_far,
      right_bottom_far,
      right_bottom_near,
      makePassiveMaterial(GREEN0))); // Right
  scene.push_back(std::make_shared<Quad>(
      left_bottom_near,
      left_bottom_far,
      right_bottom_far,
      right_bottom_near,
      makePassiveMaterial(GRAY0))); // Floor
...
Scene buildSceneCornellBox1()
{
  ...
  // scene.push_back(std::make_shared<Sphere>(Point(-0.9, 0.95, 2.0), 0.3,
  // makePassiveMaterial(YELLOW1))); scene.push_back(std::make_shared<Sphere>(Point(0, 0.95, 2.0),
  // 0.3, makePassiveMaterial(PINK1))); scene.push_back(std::make_shared<Sphere>(Point(0.9,
  // 0.95, 2.0), 0.3, makePassiveMaterial(LIGHT_BLUE1)));
  ...
  scene.push_back(
      std::make_shared<Sphere>(Point(-0.9, 0.95, 2.0), 0.3, makePassiveMaterial(YELLOW1b)));
  scene.push_back(std::make_shared<Sphere>(Point(0, 0.95, 2.0), 0.3, makePassiveMaterial(PINK1b)));
  scene.push_back(
      std::make_shared<Sphere>(Point(0.9, 0.95, 2.0), 0.3, makePassiveMaterial(LIGHT_BLUE1b)));
...
Scene buildSceneCornellBox2()
{
  ...
  scene.push_back(std::make_shared<Sphere>(
      Point(-0.9, 0.95, 2.0), 0.3, makePassiveMaterial(YELLOW2, 0.1, 0.2, LIGHT_GRAY2)));
  scene.push_back(std::make_shared<Sphere>(
      Point(0, 0.95, 2.0), 0.3, makePassiveMaterial(PINK2, 0.3, 0.2, LIGHT_GRAY2)));
  scene.push_back(std::make_shared<Sphere>(
      Point(0.9, 0.95, 2.0), 0.3, makePassiveMaterial(PURE_BLUE, 0.5, 0.4, PURE_RED)));

  // Green Spheres
  scene.push_back(std::make_shared<Sphere>(
      Point(-1.0, 0.0, 2.3), 0.175, makePassiveMaterial(WHITE, 1.0, 0, GREEN2)));
  scene.push_back(std::make_shared<Sphere>(
      Point(-0.5, 0.0, 2.3), 0.175, makePassiveMaterial(WHITE, 1.0, 0.25, GREEN2)));
  scene.push_back(std::make_shared<Sphere>(
      Point(0.0, 0.0, 2.3), 0.175, makePassiveMaterial(WHITE, 1.0, 0.5, GREEN2)));
  scene.push_back(std::make_shared<Sphere>(
      Point(0.5, 0.0, 2.3), 0.175, makePassiveMaterial(WHITE, 1.0, 0.75, GREEN2)));
  scene.push_back(std::make_shared<Sphere>(
      Point(1.0, 0.0, 2.3), 0.175, makePassiveMaterial(WHITE, 1.0, 1.0, GREEN2)));
...
Scene buildSceneSpecularSpheres()
{
  ...
  for (auto row = 0; row < 5; ++row, y += 0.5, z -= 0.2, prob += 0.25) {
    ...
    for (auto col = 0; col < 5; ++col, x += 0.5, roughness += 0.25) {
      scene.push_back(std::make_shared<Sphere>(
          Point(x, y, z), 0.175, makePassiveMaterial(WHITE, prob, roughness, GREEN2)));
    }
  }
...
cv::Mat renderFrame(const Scene& scene)
{
  ...
  for (auto i = 0; i < nb_frames; ++i) {
    ...
    const auto overall_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t0).count() / 1000.0;
